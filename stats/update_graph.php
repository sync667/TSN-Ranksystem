...
switch($_GET['serverusagechart']) {
	...
	case '3month':
		...
		break;
	default:
		$server_usage = $mysqlcon->query("SELECT `timestamp`,`clients`,`channel` FROM `$dbname`.`server_usage` ORDER BY `timestamp` DESC LIMIT 226")->fetchAll(PDO::FETCH_ASSOC);
}
